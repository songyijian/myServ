 ...
 * @LastEditTime: 2020-08-10 17:29:08
 */
"use strict"
...
module.exports = {
  fWatch: (req, res, next) => {
    ...
    const watcherItem = chokidar.watch('/Users/happyelements/github/sigmraid/200s/tbox')
    watcherItem
      .on('ready', () => log('item-监听初始化加载完成'))
      .on('error', error => log(`item-监听error: ${error}`))
      .on('change', async (path)=> {
        const fr = await fs.readFile('/Users/happyelements/github/sigmraid/200s/tbox/index.html', "utf-8");
        log(`\n 模版change：${path}`)
    ...
    const watcherADconfig = chokidar.watch('/Users/happyelements/github/sigmraid/adconfig')
    watcherADconfig
      .on('ready', () => log('config-监听初始化加载完成'))
      .on('error', error => log(`config-监听error: ${error}`))
      .on('change', async (path)=> {
        const iosfr = await fs.readJson('/Users/happyelements/github/sigmraid/adconfig/iosAdconfig.json', "utf-8");
        const anfr = await fs.readJson('/Users/happyelements/github/sigmraid/adconfig/anAdconfig.json', "utf-8");
